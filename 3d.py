...
def run_cs(arrival_rate, service_rate, servers):
    ...
    last_term = ((arrival_rate / service_rate) ** servers / np.math.factorial(servers)) * (1 / (1 - rho))
...
# STREAMLIT APP
# ============================
st.set_page_config(layout="wide", page_title="CSPC Registrar Prototype")
st.title("🏫 CSPC Registrar’s Office – 3D Simulation Prototype")
...
if st.button("▶ Run Simulation"):
    ...
    throughput = num_students / (max([s["end"] for s in students]) + 1)
    ...
    with col2:
        ...
        if cs_result:
            ...
            st.write(f"**Expected Wait (Wq):** {cs_result['Wq']:.2f} mins")
            st.write(f"**Expected System Time (W):** {cs_result['W']:.2f} mins")
    ...
    data = {
        ...
        "DES": [f"{avg_wait:.2f} min", f"{max_wait:.2f} min", "-", f"{throughput:.2f} stud/min"],
        "CS": [f"{cs_result['Wq']:.2f} min", "-", f"{cs_result['rho']*100:.1f}%", f"{cs_result['Throughput']:.2f} stud/min"]
    }
